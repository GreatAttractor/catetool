...
//!
//! TIFF image format handling code.
//!

use super::{Image, Palette, PixelFormat, bytes_per_channel, bytes_per_pixel, num_channels};
...
pub fn save_tiff(img: &Image, file_name: &str) -> Result<(), TiffError>   {
    ...
    let mut file = OpenOptions::new().read(false).write(true).create(true).truncate(true).open(file_name)?;
    ...
    field = TiffField { tag: TAG_BITS_PER_SAMPLE,
                        ftype: TAG_TYPE_WORD,
                        count: 1,
                        value: bytes_per_channel(img.pixel_format()) as u32 * 8 };
    if is_be { field.value <<= 16; }
    utils::write_struct(&field, &mut file)?;
    ...
    field = TiffField { tag: TAG_SAMPLES_PER_PIXEL,
                        ftype: TAG_TYPE_WORD,
                        count: 1,
                        value: num_channels(img.pixel_format()) as u32 };
    if is_be { field.value <<= 16; }
    utils::write_struct(&field, &mut file)?;
    ...
    field = TiffField { tag: TAG_STRIP_BYTE_COUNTS,
                        ...
                        count: 1,
                        value: img.bytes_per_line() as u32 * img.height() }; // There is only one strip for the whole image
    utils::write_struct(&field, &mut file)?;
